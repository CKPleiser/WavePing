...
class WaveScheduleScraper {
  ...
  _getFallbackHTML() {
    ...
    const dateLabel = today.format('ddd Do MMM');
  ...
  _dateFromLabel(label) {
    ...
    let candidate = dayjs.tz(dateString, this.TIMEZONE);
    
    // If the candidate date is more than 3 months in the past, assume it's next year
    const threeMonthsAgo = base.subtract(3, 'month');
    if (candidate.isBefore(threeMonthsAgo)) {
      candidate = dayjs.tz(`${currentYear + 1}-${String(monthIndex + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`, this.TIMEZONE);
    }
  ...
  _to24(timeStr) {
    ...
    const [, hoursStr, minutes, ampmRaw] = match;
  ...
  filterSessionsForUser(sessions, userLevels = [], userSides = [], userDays = [], skipDayFilter = false) {
    ...
    console.log('First session sample:', sessions[0] ? { level: sessions[0].level, side: sessions[0].side, session_name: sessions[0].session_name } : 'none');
    console.log('User levels:', userLevels);
    console.log('User sides:', userSides);
    console.log('User days:', userDays);
    console.log('Skip day filter:', skipDayFilter);
    ...
    if (userSides.length > 0) {
      console.log('🏄 Before side filtering:', filtered.length, 'sessions');
      filtered = filtered.filter(session => {
        ...
        const match = userSides.includes(session.side);
        console.log(`Session ${session.session_name}: side=${session.side}, userSides=${JSON.stringify(userSides)}, match=${match}`);
        return match;
      });
      console.log('🏄 After side filtering:', filtered.length, 'sessions');
    }
    ...
    if (userDays.length > 0 && !skipDayFilter) {
      ...
      // Only filter if the current day is NOT in user's preferred days

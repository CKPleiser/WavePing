  ...
  filterSessionsForUser(sessions, userLevels = [], userSides = [], userDays = [], skipDayFilter = false) {
    ...
    if (userSides.length > 0) {
      filtered = filtered.filter(session => {
        ...
        // Convert session.side format to match user preference format
        // Session sides are 'L'/'R', user sides might be 'Left'/'Right' or 'L'/'R'
        const sessionSide = session.side;
        const normalizedUserSides = userSides.map(side => {
          if (side === 'Left') return 'L';
          if (side === 'Right') return 'R';
          return side; // Keep 'L', 'R', 'Any' as is
        });
        
        return normalizedUserSides.includes(sessionSide);
      });
    }
